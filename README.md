...
| `table` | 表格：定宽/均分/内容自适应列宽、灰底加粗表头、单元格合并（跨列/跨行，含表头行）、边框（宽度/颜色/样式：全框线/关闭/仅横线/仅竖线）与内边距、跨页自动重复表头（可关闭）、超高行/跨行块跨页拆分 |
...
| `jsongen` | JSON 描述 → PDF 文档模板引擎（格式见 `doc/json-format.md`）：段落/表格/多栏、页码回绘与分节重计数（节可单独关闭页码）；字体经 `FontRegistry` 代码层注册，JSON 禁止携带字体路径 |
